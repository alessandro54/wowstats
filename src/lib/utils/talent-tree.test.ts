...
describe("buildNodeMap", () => {
  it("creates one node per unique node_id", () => {
    const talents = [
      makeTalent(1, 10, 1, 1, 80),
      makeTalent(2, 20, 2, 1, 60),
    ]
  ...
  it("marks a node as choice when two talents share the same node_id", () => {
    const talents = [
      makeTalent(1, 10, 1, 1, 70),
      makeTalent(2, 10, 1, 1, 30),
    ]
    const map = buildNodeMap(talents)
    const node = map.get(10)!
  ...
  it("sets primary to the talent with the highest usage_pct", () => {
    const talents = [
      makeTalent(1, 10, 1, 1, 30),
      makeTalent(2, 10, 1, 1, 80),
    ]
    const map = buildNodeMap(talents)
  ...
  it("primary updates even when the higher-pct talent is added second", () => {
    const talents = [
      makeTalent(1, 10, 1, 1, 20),
      makeTalent(2, 10, 1, 1, 95),
    ]
    const map = buildNodeMap(talents)
...
describe("buildTopNodeIds", () => {
  const nodes = [
    {
      ...buildNodeMap([
        makeTalent(1, 1, 1, 1, 90),
      ]).get(1)!,
    },
    {
      ...buildNodeMap([
        makeTalent(2, 2, 1, 2, 70),
      ]).get(2)!,
    },
    {
      ...buildNodeMap([
        makeTalent(3, 3, 1, 3, 50),
      ]).get(3)!,
    },
    {
      ...buildNodeMap([
        makeTalent(4, 4, 1, 4, 30),
      ]).get(4)!,
    },
  ]
  ...
  it("respects maxRank cost — a rank-2 node costs 2 points", () => {
    const twoRankNode = buildNodeMap([
      makeTalent(5, 5, 1, 5, 95, 2),
    ]).get(5)!
    const ids = buildTopNodeIds(
      [
        twoRankNode,
        ...nodes,
      ],
...
describe("hasTreeData", () => {
  it("returns true when at least one talent has row and col", () => {
    const talents = [
      makeTalent(1, 10, null, null, 80),
      makeTalent(2, 20, 2, 3, 60),
    ]
