...
class FelFlame {
  ...
  reset(ctx: CanvasRenderingContext2D, fresh: boolean) {
    ...
    g.addColorStop(0, "rgba(200,255,120,0.9)")
    g.addColorStop(0.25, "rgba(50,210,10,0.7)")
    g.addColorStop(0.6, "rgba(10,130,0,0.35)")
...
class FelEmber {
  ...
  reset(ctx: CanvasRenderingContext2D, fresh: boolean) {
    ...
    g.addColorStop(0, "rgba(160,255,60,0.9)")
    g.addColorStop(0.5, "rgba(30,200,0,0.4)")
...
export function runFelFire(ctx: CanvasRenderingContext2D, W: number, H: number): () => void {
  const flames = Array.from(
    {
      length: 28,
    },
    () => new FelFlame(ctx, W, H, true),
  )
  const embers = Array.from(
    {
      length: 45,
    },
    () => new FelEmber(ctx, W, H, true),
  )
  ...
  function drawGlow() {
    ...
    g.addColorStop(0, `rgba(0,120,0,${0.07 + pulse * 0.04})`)
    g.addColorStop(0.5, `rgba(0,60,0,${0.03 + pulse * 0.02})`)
    g.addColorStop(1, "rgba(0,0,0,0)")
    ctx.fillStyle = g
