...
class HolyFlame {
  ...
  reset(ctx: CanvasRenderingContext2D, fresh: boolean) {
    ...
    g.addColorStop(0, "rgba(255,255,220,0.95)") // white-gold core
    g.addColorStop(0.2, "rgba(255,220,60,0.8)") // bright gold
    g.addColorStop(0.55, "rgba(220,140,10,0.4)") // amber mid
...
class GoldenEmber {
  ...
  reset(ctx: CanvasRenderingContext2D, fresh: boolean) {
    ...
    g.addColorStop(0.5, "rgba(255,180,20,0.45)")
...
export function runHolyFire(ctx: CanvasRenderingContext2D, W: number, H: number): () => void {
  const flames = Array.from(
    {
      length: 28,
    },
    () => new HolyFlame(ctx, W, H, true),
  )
  const embers = Array.from(
    {
      length: 45,
    },
    () => new GoldenEmber(ctx, W, H, true),
  )
  ...
  function drawGlow() {
    ...
    g.addColorStop(0, `rgba(200,130,0,${0.08 + pulse * 0.05})`)
    g.addColorStop(0.5, `rgba(120,70,0,${0.03 + pulse * 0.02})`)
    g.addColorStop(1, "rgba(0,0,0,0)")
    ctx.fillStyle = g
