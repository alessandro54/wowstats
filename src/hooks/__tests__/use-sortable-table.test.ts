...
const comparators = {
  score: (a: Item, b: Item) => b.score - a.score,
  rating: (a: Item, b: Item) => b.rating - a.rating,
}

describe("useSortableTable", () => {
  it("sorts by the default key in descending order by default", () => {
    const { result } = renderHook(() => useSortableTable(entries, comparators, "score", false))
  ...
  it("sorts by the default key in ascending order when defaultAsc is true", () => {
    const { result } = renderHook(() => useSortableTable(entries, comparators, "score", true))
  ...
  it("toggles sort direction when the same key is clicked twice", () => {
    ...
    const namesAsc = result.current.sorted.map((e) => e.name)
    ...
    const namesDesc = result.current.sorted.map((e) => e.name)
