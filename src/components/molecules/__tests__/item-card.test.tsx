...
const defaultProps = {
  slot: "HEAD",
  entries: [makeItem("HEAD", "Helm of Glory", 85.0)],
  enchants: new Map<string, EnchantGroup>(),
  fiberGems: [] as MetaGem[],
  activeColor: "#c79c6e",
  pillStyle: { "--pill-color": "#c79c6e" } as React.CSSProperties,
}
...
describe("itemCard", () => {
  ...
  it("renders an empty div when entries is undefined", () => {
    const { container } = render(
      <ItemCard {...defaultProps} entries={undefined} />,
    )
  ...
  it("renders an empty div when entries is empty", () => {
    const { container } = render(
      <ItemCard {...defaultProps} entries={[]} />,
    )
    expect(container.querySelector("[data-testid=\"tooltip-wrapper\"]")).toBeNull()
  })

  it("shows CRAFTED badge on crafted items", () => {
    const crafted = [makeItem("HEAD", "Crafted Helm", 72.0, "EPIC", true)]
  ...
  it("renders the enchant name when provided", () => {
    const enchants = new Map<string, EnchantGroup>([
      ["HEAD", { slot: "HEAD", entries: [makeEnchant("HEAD", "Enchant Helm - Radiant Power", 65.0)] }],
    ])
  ...
  it("does not show enchant when no enchant group for slot", () => {
    const enchants = new Map<string, EnchantGroup>([
      ["CHEST", { slot: "CHEST", entries: [makeEnchant("CHEST", "Chest Enchant", 50.0)] }],
    ])
    const { container } = render(<ItemCard {...defaultProps} enchants={enchants} />)
  ...
  it("shows fiber gem badge on Reshii Wraps when fiber gems provided", () => {
    const reshii = [makeItem("HANDS", "Reshii Wraps of Insanity", 55.0)]
    const fiber = [makeGem("TINKER", "Jeweler's Setting", 80.0)]
    const { container } = render(
      <ItemCard
        {...defaultProps}
        slot="HANDS"
